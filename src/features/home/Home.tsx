  ...
  useEffect(() => {
    ...
    const prefetchRoutes = async () => {
      try {
        ...
        // Silent fail as this is an optimization
      }
    };
    ...
    const startPrefetch = () => {
      ...
      if (win.requestIdleCallback) {
        idleId = win.requestIdleCallback(prefetchRoutes, { timeout: 2000 });
      } else {
        timeoutId = win.setTimeout(prefetchRoutes, 3000);
      }
    };
    ...
    // Trigger on interaction or after a safe delay
    window.addEventListener('mousemove', handleUserIntent, { once: true });
    window.addEventListener('touchstart', handleUserIntent, { once: true });
    window.addEventListener('scroll', handleUserIntent, { once: true });
  ...
  return (
    ...
    >
      <SEO
        ...
        keywords="3D animation Cairo, motion graphics Egypt, cinematic 3D studio, architectural visualization Cairo, CGI advertising Egypt, 3D product rendering, anamorphic 3D production"
        schema={[
          Schemas.organization(),
          Schemas.videoObject({
            name: 'Geometric Studio Master Showreel',
            description: 'Enter the world of Geometric Studio — where precision meets imagination in every pixel.',
            thumbnailUrl: 'https://geometric-studios.com/assets/images/Intro_Page.webp',
      ...
      <section ref={heroRef} className="relative min-h-[80vh] md:h-screen flex items-center justify-center overflow-hidden" aria-label="Hero - Geometric Studio Showreel">
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-background/90 via-background/20 to-transparent opacity-80 z-0" />
        </m.div>
          ...
          <div className="flex flex-col lg:flex-row items-center lg:items-start justify-between gap-12 lg:gap-20">
            <div className="w-full lg:w-3/5 space-y-6 md:space-y-10 text-center lg:text-left">
              ...
              >
                ...
                <Link
                  to="/contact"
                  onMouseEnter={() => import('@/features/contact')}
            ...
            <div className="w-full lg:w-3/5 space-y-12">
              ...
              <SectionReveal delay={0.3}>
                <div className="pt-4">
                  <Link 
                    to="/about" 
                    onMouseEnter={() => import('@/features/about')}
                    className="text-[10px] font-ibm tracking-[0.3em] uppercase text-neon-yellow hover:text-white transition-colors"
                  >
        ...
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-24 gap-8">
            ...
            <SectionReveal delay={0.2}>
              <Link 
                to="/services" 
                onMouseEnter={() => import('@/features/services')}
                className="group flex items-center gap-6 text-[10px] font-ibm tracking-[0.3em] uppercase text-creamy-white/50 hover:text-neon-yellow transition-all duration-500"
              >
